...
#define SHADOWMAP_RES (512.0)
// set to a number between 1 and 16. values from 2 to 5 are normal, values above 5 are silly. 1 means no filtering.
// ideally the dither will allow you to go lower
#define FILTER_SIZE 4

// controls penumbra width - higher = softer shadows. 1.0 is default, 1.5-2.0 gives noticeably softer edges.
#define STRIDE_MIN_PX 2
...
// plane-dependent filtering
    ...
    // Early bail: sample corners first to detect fully lit/shadowed areas
    float c1 = shadow2DProj(tex, coord + offs*(-ends) + offs2*(-ends)).r;
    float c2 = shadow2DProj(tex, coord + offs*( ends) + offs2*(-ends)).r;
    float c3 = shadow2DProj(tex, coord + offs*(-ends) + offs2*( ends)).r;
    float c4 = shadow2DProj(tex, coord + offs*( ends) + offs2*( ends)).r;

    // Fully lit - bail early
    if (c1 == 1.0 && c2 == 1.0 && c3 == 1.0 && c4 == 1.0)
        return 1.0;

    // Fully shadowed - bail early
    if (c1 == 0.0 && c2 == 0.0 && c3 == 0.0 && c4 == 0.0)
        return 0.0;

    // Penumbra region - do full filtering
    float amount = c1 + c2 + c3 + c4;
    float norm = 4.0;
    ...
    for (float y = -ends; y < ends + 0.1; y++)
    {
        ...
        {
            // Skip corners we already sampled
            if ((x == -ends || x == ends) && (y == -ends || y == ends))
                continue;
